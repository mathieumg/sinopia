...
function Storage(config) {
  ...
  self.locker = new Locker(config, self.logger)
...
//
Storage.prototype.add_package = function add_package(name, metadata, callback) {
  var self = this
  ...
  function publish_package(callback) {
...
//
Storage.prototype.add_version = function add_version(name, version, metadata, tag, callback) {
  var self = this
  self.locker.writeLock(name, callback, function(unlock) {
    self.local.add_version(name, version, metadata, tag, unlock)
  }) 
}
...
// Used storages: local (write)
//
Storage.prototype.merge_tags = function merge_tags(name, tag_hash, callback) {
  var self = this
  self.locker.writeLock(name, callback, function(unlock) {
    self.local.merge_tags(name, tag_hash, unlock)
  })
}
...
// Used storages: local (write)
//
Storage.prototype.replace_tags = function replace_tags(name, tag_hash, callback) {
  var self = this
  self.locker.writeLock(name, callback, function(unlock) {
    self.local.replace_tags(name, tag_hash, unlock)
  })
}
...
// Used storages: local (write)
//
Storage.prototype.change_package = function change_package(name, metadata, revision, callback) {
  var self = this
  self.locker.writeLock(name, callback, function(unlock) {
    self.local.change_package(name, metadata, revision, unlock)
  })
}
...
// Used storages: local (write)
//
Storage.prototype.remove_package = function remove_package(name, callback) {
  var self = this
  self.locker.writeLock(name, callback, function(unlock) {
    self.local.remove_package(name, unlock)
  })
}
...
//
// Used storages: locReferenceError: lock is not definedal (write)
//
Storage.prototype.remove_tarball = function remove_tarball(name, filename, revision, callback) {
  var self = this
  self.locker.writeLock(name, callback, function(unlock) {
    self.local.remove_tarball(name, filename, revision, unlock)
  })
}
//
...
// Used storages: local (write)
//
Storage.prototype.add_tarball = function(name, filename, callback) {
  var self = this
  var stream

  // cleanup stream if lock is automatically released due to a timeout
  function unlockCallback(err) {
    if (err) {
  ...
  self.locker.writeLock(name, unlockCallback, function(unlock) {
     stream = self.local.add_tarball(name, filename)

     stream.on('error', unlock)
     stream.on('success', unlock)
     stream.on('abort', unlock)
...
//
Storage.prototype.get_tarball = function get_tarball(name, filename, callback) {
  
  var self = this
  var stream

  // error handling if lock is automatically released due to a timeout
  function unlockCallback(err) {
    if (err) {
      if (stream) stream.abort()
      stream = null
  ...
  self.locker.readLock(name, unlockCallback, function(unlock) {
    ...
    rstream.on('error', function(err) {
      ...
      self.local.get_package(name, function(err, info) {
        if (!err && info._distfiles && info._distfiles[filename] != null) {
          ...
          serve_file(info._distfiles[filename], stream)
  ...
  function serve_file(file, stream) {
    var uplink = null
...
//
Storage.prototype.get_package = function get_package(name, options, callback) {
  if (typeof(options) === 'function') callback = options, options = {}

  var self = this

  self.locker.readLock(name, callback, function(unlock) {
    ...
    self.local.get_package(name, options, function(err, data) {
      ...
      self._sync_package_with_uplinks(name, data, options, function(err, result, uplink_errors) {
        ...
        unlock(null, result, uplink_errors)
      })
    })
  })
