...
function errorLockExpired() {
  return new Error("Operation took longer than " + this.timeout + " ms to finish (config.timeout)")
}

function tryRelease() {
  if (Math.abs(Date.now() - this.activeLock.whenObtained) > this.timeout) {
    this.activeLock.release(errorLockExpired())
  } else {
...
Locker.prototype.readLock = function(key, origCb, cb) {
  return this.rwlock.readLock(key, getRelease.call(this, origCb, cb));
}

Locker.prototype.writeLock = function(key, origCb, cb) {
  return this.rwlock.writeLock(key, getRelease.call(this, origCb, cb))
}
